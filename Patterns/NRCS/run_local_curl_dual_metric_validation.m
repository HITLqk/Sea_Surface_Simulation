...
hLiteratureMedian = yline(referenceMedian, '--', ...
    ...
    'DisplayName', sprintf('Literature median: %.2f dB', ...
    referenceMedian));
...
title('(a) Continuous Model Response'); grid on;
style_axes(gca);
...
hModelSamples = scatter(modelX, modelValues, 25, [0.22 0.46 0.72], ...
    'filled', 'MarkerFaceAlpha', 0.52, 'DisplayName', 'Model samples');
...
xticklabels({'Literature','Present model'});
ylabel('G_b (dB)');
title('(b) Distribution-Level Comparison'); grid on;
text(1.5, 9.55, sprintf('|median difference| = %.2f dB', ...
    abs(diff(medians))), 'HorizontalAlignment', 'center', ...
    'Color', [0.2 0.2 0.2]);
