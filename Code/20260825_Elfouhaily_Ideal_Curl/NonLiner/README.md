...
1. Elfouhaily 一阶波幅已经包含风速效应，默认二阶无量纲增益固定为 1，不再额外使用经验风速指数调节曲线。
2. 风向通过波矢在主风向上的投影调制二阶分量。
3. 二阶输入和输出分别采用固定的 `6 rad/m` 与 `12 rad/m` 物理截止，不再跟随 Nyquist 波数变化。
4. `eta_1^2` 在 3/2 扩展频谱网格上计算后裁回，避免二次卷积混叠。
5. 水平 Riesz 位移使用同一固定输入带宽，使波峰沿传播方向变陡，并通过雅可比线搜索自动避免网格折叠。
