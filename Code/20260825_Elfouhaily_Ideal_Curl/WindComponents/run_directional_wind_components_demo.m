...
% Plot the same complete 128 m x 128 m component as a physical 3-D sea.
% The z data remain in metres; only the plot-box ratio is enlarged so the
% approximately +/-0.3 m relief remains visible over the large domain.
componentLimit = max(abs(surfaceData.ZWindComponent),[],'all');
figComponent3D = new_figure(cfg.output.figureVisible,[90 90 1280 820]);
surf(surfaceData.XLinear,surfaceData.YLinear, ...
    surfaceData.ZWindComponent,surfaceData.ZWindComponent, ...
    'EdgeColor','none');
shading interp;
axis tight vis3d;
zlim([-componentLimit,componentLimit]);
pbaspect([1 1 0.38]); view(42,30); grid on;
camproj orthographic; camzoom(0.68);
camlight('headlight'); lighting gouraud; material dull;
rotate3d on;
xlabel('x (m)'); ylabel('y (m)'); zlabel('z (m)');
title('Complete 3-D Directional Short-scale Wind-wave Sea');
colormap(turbo); clim([-componentLimit,componentLimit]);
componentAxes3D = gca;
componentColorbar3D = colorbar;
set(componentAxes3D,'Position',[0.07 0.10 0.75 0.82]);
set(componentColorbar3D,'Position',[0.87 0.16 0.022 0.70]);
...
exportgraphics(figComponent,fullfile(cfg.output.outputDirectory, ...
    ...
    'BackgroundColor','white');
exportgraphics(figComponent3D,fullfile(cfg.output.outputDirectory, ...
    '03_directional_short_wind_component_3d.png'),'Resolution',180, ...
    'BackgroundColor','white');
exportgraphics(figCombined,fullfile(cfg.output.outputDirectory, ...
    '04_combined_wind_component_surface.png'),'Resolution',180, ...
    'BackgroundColor','white');
exportgraphics(figDetail,fullfile(cfg.output.outputDirectory, ...
    '05_selected_crest_3d_detail.png'),'Resolution',200, ...
    'BackgroundColor','white');
exportgraphics(figSection,fullfile(cfg.output.outputDirectory, ...
    '06_selected_crest_directional_section.png'),'Resolution',200, ...
    'BackgroundColor','white');
