...
4. 使用原方法的指数权重 `exp(-abs(u)/amplitudeCurl)` 和局部 `(u,z)` 旋转生成卷曲。
5. 沿波峰方向增加紧支撑余弦窗，使卷曲只作用于有限波峰段。每个二维网格点保留自身 Elfouhaily 高程，因此不再是将一条曲线横向复制得到的柱状海面。

默认卷曲参数直接采用参考脚本的核心数值：`amplitudeCurl=0.2 m`、`curlMultiplier=0.5 rad`、`pivotDepth=1.5 m`。程序同时检查传播方向映射 `du_final/du`，没有负值时会判定为仅变形、未形成几何卷曲。
