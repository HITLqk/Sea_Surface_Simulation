...
[Psi,saturationMeta] = apply_high_wind_saturation(Psi,K,cfg.sea);
...
surfaceData.saturationMeta = saturationMeta;
...
function [Psi,meta] = apply_high_wind_saturation(Psi,K,sea)
meta.enabled = false;
meta.mu = 0;
meta.gain = 1;
if ~isfield(sea,'highWindSaturation') || ...
        ~sea.highWindSaturation.enabled
    return;
end

closure = sea.highWindSaturation;
assert(numel(closure.windSpeeds) == numel(closure.mu) && ...
    numel(closure.windSpeeds) == numel(closure.gain) && ...
    numel(closure.windSpeeds) >= 2, ...
    'High-wind closure requires paired wind-speed and mu arrays.');
mu = interp1(closure.windSpeeds(:),closure.mu(:),sea.U10, ...
    'pchip','extrap');
mu = max(mu,0);
gain = interp1(closure.windSpeeds(:),closure.gain(:),sea.U10, ...
    'pchip','extrap');
gain = max(gain,1);
kMaximum = sea.maximumWavenumber;
assert(isfinite(kMaximum) && kMaximum > 0, ...
    'High-wind closure requires a finite synthesis maximum wavenumber.');
attenuation = gain*exp(-mu*(K/kMaximum).^ ...
    closure.wavenumberExponent);
Psi = Psi.*attenuation;
meta.enabled = true;
meta.mu = mu;
meta.gain = gain;
meta.wavenumberExponent = closure.wavenumberExponent;
end
...
if cfg.sea.highWindSaturation.enabled
    assert(cfg.sea.highWindSaturation.wavenumberExponent > 0, ...
        'Saturation wavenumber exponent must be positive.');
end
